...
class DataUpcommingViewModel: ViewModel() {

    private val _dataState = MutableLiveData<ApiState<List<Result>>>()
    val dataState: LiveData<ApiState<List<Result>>> get() = _dataState
    fun loadUpcommingdata(page: Int) {  // Pass page as a parameter
        val dataService = ApiManager.getDataService()
        viewModelScope.launch {
            try {
                val dataResponse = dataService.getUpcomingMovies(page) // No need for execute()
